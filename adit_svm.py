...
def features_svm(wid, sen, w2v,tags):
    ...
    no_prev_words = 2
    for i in reversed(range(0,no_prev_words+1)):
    ...
    no_next_words = 2
    for i in range(1,no_next_words+1):
        if sen[wid+i][0] not in keys:
            vec = np.zeros(w2v.vector_size)
            ft.append(vec)
        else:
    ...
    no_prev_tags = 2
    for i in reversed(range(1,no_prev_tags+1)):
        tag = sen[wid-i][1]
    ...
    if len(sen[wid][0]) > 5:
        if sen[wid][0][-4:].lower() in ["ship","able","wise","ward","ible","less","ious","ness","sion","ment"] or sen[wid][0][-3:].lower() in ["eer","ion","ant","ary","ing","ize","ise","ful","ous","ive","ity"] or sen[wid][0][-2:].lower() in ["er","or","ed","en","er","al","ly","ic","th"]:
            ft.append([1])
        else: 
            ft.append([0])
    else:
...
for each_sent in tqdm(test_set):
    ...
    for j in range(2,len(each_sent)-2):
        x_text.append(features_svm(j, each_sent, w2v_words, tags_list))
...
for each_sent in tqdm(train_set):
    ...
    for j in range(2,len(each_sent)-2):
        fet_train.append(features_svm(j, each_sent, w2v_words, tags_list))
...
grad_svm = SVM()
losses_sgd = grad_svm.train(X_train, y_train, lr=0.1,reg = 0, num_iters=50000, batch_size=256)
